...
func TestIsOutdatedCert(t *testing.T) {
	...
	_, _, err = cert.GenCertPair(
		...
		extraDomains[:2], false, cert.DefaultAutogenValidity, false, keyRing,
	)
	if err != nil {
		t.Fatal(err)
	}
...
func TestIsOutdatedPermutation(t *testing.T) {
	...
	_, _, err = cert.GenCertPair(
		...
		extraDomains[:], false, cert.DefaultAutogenValidity, false, keyRing,
	)
	if err != nil {
		t.Fatal(err)
	}
...
func TestTlsConfig(t *testing.T) {
	...
	_, _, err = cert.GenCertPair(
		...
		[]string{extraDomains[0]}, cert.DefaultAutogenValidity, false,
		keyRing, false,
	)
	if err != nil {
		t.Fatal(err)
...
func TestEncryptedTlsConfig(t *testing.T) {
	...
	_, _, err = cert.GenCertPair(
		"lnd autogenerated cert", certPath, keyPath, []string{extraIPs[0]},
		[]string{extraDomains[0]}, cert.DefaultAutogenValidity, true,
		keyRing, false,
	)
	if err != nil {
		t.Fatal(err)
...
// TestTLSDisableAutofill checks that setting the --tls-disable-autofill flag
// does not add interface ip addresses or hostnames to the cert.
func TestTLSDisableAutofill(t *testing.T) {
	tempDir, err := ioutil.TempDir("", "certtest")
	if err != nil {
