...
func Main(cfg *Config, lisCfg ListenerCfg, shutdownChan <-chan struct{}) error {
	...
	if cfg.TLSEncryptKey {
		...
		err = os.Remove(cfg.TLSCertPath + ".tmp")
		if err != nil {
...
func getEphemeralTLSConfig(cfg *Config, keyRing keychain.KeyRing) (*tls.Config,
	*credentials.TransportCredentials, string, error) {
	...
	certBytes, keyBytes, err := cert.GenCertPair(
		...
		tmpValidity, false, keyRing, cfg.TLSDisableAutofill,
	)
	if err != nil {
		return nil, nil, "", err
	}
	...
	*credentials.TransportCredentials, string, error) {
	...
	if !fileExists(cfg.TLSCertPath) && !fileExists(cfg.TLSKeyPath) {
		...
		_, _, err := cert.GenCertPair(
			...
			cert.DefaultAutogenValidity, cfg.TLSEncryptKey, keyRing, cfg.TLSDisableAutofill,
		)
		if err != nil {
			return nil, nil, "", err
		}
	...
	if cfg.ExternalSSLDomain != "" {
		...
		if cfg.TLSAutoRefresh {
			refresh, err = cert.IsOutdated(
				parsedCert, cfg.TLSExtraIPs, cfg.TLSExtraDomains, cfg.TLSDisableAutofill,
			)
			if err != nil {
				return nil, nil, "", err
			}
