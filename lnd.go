...
func Main(cfg *Config, lisCfg ListenerCfg, shutdownChan <-chan struct{}) error {
	...
	var nilKeyRing keychain.KeyRing
	if cfg.TLSEncryptKey {
		tlsCfg, restCreds, restProxyDest, err = getEphemeralTLSConfig(cfg, nilKeyRing)
	} else {
		tlsCfg, restCreds, restProxyDest, err = getTLSConfig(cfg, nilKeyRing)
	}
	if err != nil {
	...
	// If encryption is set, load the real one now since the wallet is unlocked
	if cfg.TLSEncryptKey {
...
// cert in memory for the gRPC server and credentials  and a proxy
// destination for the REST reverse proxy. These get removed when the
// WalletUnlocker goes away. If --tlsverifywebook is set we also call the
// webhook to notify of a new certificate for validation
func getEphemeralTLSConfig(cfg *Config, keyRing keychain.KeyRing) (*tls.Config,
	*credentials.TransportCredentials, string, error) {
	...
	certBytes, keyBytes, err := cert.GenCertPair(
		"lnd autogenerated cert", "",
		"", cfg.TLSExtraIPs, cfg.TLSExtraDomains,
		tmpValidity, false, keyRing,
	)
	if err != nil {
		return nil, nil, "", err
	}
	...
	/*
		// If there's a webhook defined we need to notify it
		if cfg.TLSVerifyWebhook != "" {
			webhookEndpoint, err := validateWebhookEndpoint(cfg.TLSVerifyWebhook)
			timestamp := time.Now().Format("2009-01-03T18:15:05Z")
			b64TlsCert := base64.StdEncoding.EncodeToString(certData.Certificate[0])
			payload := []byte(fmt.Sprintf(`{"timestamp":"%s","tls_cert":"%s"}`, timestamp, b64TlsCert))
			req, err := http.NewRequest("POST", webhookEndpoint, bytes.NewBuffer(payload))
			req.Header.Set("Content-Type", "application/json")
			if cfg.TLSVerifyWebhookSecret != "" {
				req.Header.Set("Authorization", fmt.Sprintf("Bearer %s", cfg.TLSVerifyWebhookSecret))
			}
			client := &http.Client{}
			...
			if resp.StatusCode >= 200 && resp.StatusCode <= 299 {
				fmt.Println("Did not receive a 200 code")
			}
...
func getTLSConfig(cfg *Config, keyRing keychain.KeyRing) (*tls.Config, *credentials.TransportCredentials,
	string, error) {
	...
	if !bytes.HasPrefix(keyBytes, privateKeyPrefix) {
		if !cfg.TLSEncryptKey {
			return nil, nil, "", fmt.Errorf("TLS Private Key is encrypted but the --tlsencryptkey flag was not passed.")
		}
...
// validateWebhookEndpoint checks to make sure the specified Webhook
// endpoint is indeed a valid DNS name. If so, it returns it with
// no errors, else an error is returned.
