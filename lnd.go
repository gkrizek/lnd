...
func Main(cfg *Config, lisCfg ListenerCfg, shutdownChan <-chan struct{}) error {
	...
	if cfg.TLSEncryptKey {
		tlsCfg, restCreds, restProxyDest, err = getEphemeralTLSConfig(cfg)
	} else {
	...
	// TODO check here if encryption is enabled.
	// TODO If so, load the TLSConfig for the main cert
...
func getEphemeralTLSConfig(cfg *Config) (*tls.Config,
...
func getTLSConfig(cfg *Config) (*tls.Config, *credentials.TransportCredentials,
	...
	// TODO First check if the file is in plaintext or encrypted.
	// TODO if encrypted, Decrypt the tls key buffer first.
	certData, parsedCert, err := cert.LoadCert(
		certBytes, keyBytes,
	)
	if err != nil {
