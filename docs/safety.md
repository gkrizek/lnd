...
seed then writes the encrypted blob to disk.
...
`WalletUnlocker` service without TLS. To circumvent this problem, LND use a 
...
do this in `lncli` with the `--tls_tmp_cert` flag that's availabe on the 
`create`, `unlock`, and `changepassword` commands. If you aren't able to run
`lncli` on the host `lnd` is running on, then you'll need to copy the 
temporary certificate from the host onto whatever device you're using. Ignoring
TLS certificate verification is considered insecure and not recommended.
